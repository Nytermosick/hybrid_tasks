...
# Make sure the local mujoco config is resolved first, not deploy/config.py.
...
class ElasticBand:
    ...
    def __init__(self):
        ...
        self.q_des = np.array([1.0, 0.0, 0.0, 0.0])  # w, x, y, z

    def Advance(self, x, dx, q, omega):
        """
        Args:
        ...
        # q_err = q_des * q_inv
        q_err = np.zeros(4)
        mujoco.mju_mulQuat(q_err, self.q_des, q_inv)

        # orientation error (small-angle approx)
        e_R = 2.0 * np.sign(q_err[0]) * q_err[1:]

        tau = 200 * e_R - 50 * omega
